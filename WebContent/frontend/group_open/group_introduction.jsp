			...
			<div class="container">
				<div class="row">
					...
					<div class="col-xs-12 col-sm-10">
						...
						<div class="col-xs-12 col-sm-6">
							<div><span class="text-muted"> 開團名稱</span>:${group_openVO.group_name}</div>	
							<br>
							<div><span class="text-muted"> 商品名稱</span>
							...
							<br>
							<div><span class="text-muted" style="text-decoration:line-through";> 商品價格</span>
											<c:forEach var="goodsVO" items="${goodsSvc.getAll()}">		
												<c:if test="${group_openVO.goods_no==goodsVO.goods_no}">
							...
							<br>
							<div><span class="text-muted">折扣後商品價格</span><span style="font-size:18px"><font color="#750000" >:${group_openVO.group_price}</font></span></div>
							<br>	
							<div><span class="text-muted"> 目前跟團人數</span>:
											<c:forEach var="group_openmap" items="${group_openSvc.member_count}">
												<c:if test="${group_openVO.group_no==group_openmap.key}">
	                   					<span style="font-size:16px">${group_openmap.value}</span>		
                    							</c:if>	
											</c:forEach>												
							</div>	
							<br>
							<div>				<span class="text-muted"> 售出數量</span>:
												<c:forEach var="group_membermap" items="${group_memberSvc.group_quantity}">
												<c:if test="${group_openVO.group_no==group_membermap.key}">
	                   						<span style="font-size:16px">${group_membermap.value}</span>
                    							</c:if>	
                    							</c:forEach>
							</div>
				...
				<div class="row">
					...
					<div class="col-xs-12 col-sm-9">
					  ...
					  <div class="panel-heading">
						...
						<%-- <h5><span style="text-align:left;">${forumVO.forum_time}</span></h5>	 --%>					 	   
						 	<%--  <h5> <fmt:formatDate value="${forumVO.forum_time}" pattern="yyyy-MM-dd HH:mm"/></h5> --%>
						 	  <h5> <fmt:formatDate value="${forumVO.forum_time}" pattern="HH:mm yyyy-MM-dd"/></h5>
							</span>
